...
namespace lanczos {
    ...
    namespace io {
        ...
        template<>
        inline MatrixXd read_input_file<MatrixXd>(const std::string& filename) {
        ...
        template<>
        inline SparseMatrixXd read_input_file<SparseMatrixXd>(const std::string& filename) {
            std::ifstream in(filename);
            if (!in) {
                throw std::runtime_error("could not open file " + filename);
            }
